...
cv::Point2f recursive_bezier(const std::vector<cv::Point2f>& control_points, float t) {
    ...
    std::vector<cv::Point2f> secondary_points;
    for (int i = 0; i < len - 1; ++i) {
        secondary_points.push_back(control_points[i] * (1 - t) + control_points[i + 1] * t);
    }
    return recursive_bezier(secondary_points, t);
}
...
void bezier(const std::vector<cv::Point2f>& control_points, cv::Mat& window) {
    ...
    for (double t = 0.0; t <= 1.0; t += 0.001) {
        ...
        std::vector<float> ds = {};
        for (int i = 0; i < 4; i++) {
            float d = max_d - std::sqrt(std::pow(p.x - ps[i].x - 0.5, 2) + std::pow(p.y - ps[i].y - 0.5, 2));   // 越近则越绿，使用与根号2的差表示接近程度
            ds.push_back(d);
            sum_d += d;
        };
